...
python3 -m gpti_data discover ./data/seeds/firms_full.json

# 2. Crawl for current data
python3 -m gpti_data crawl 100

# 3. Generate production snapshot
python3 -m gpti_data export-snapshot
```
...
- CLI: `/opt/gpti/gpti-data-bot/src/gpti_data/cli.py`

**Data:**
- Snapshots: MinIO bucket `gpti-snapshots`
- Database: PostgreSQL `gpti_data`
